...
import type { RecordDTO, RecordInput } from '../api/endpoints'
import { recordTypes as recordTypeConfig } from '../data/recordTypes'
...
function recordTypesLabel(type: string): string {
  const all = [...recordTypeConfig.high, ...recordTypeConfig.health, ...recordTypeConfig.life]
...
export const services = {
  async getFamily(): Promise<APIResponse<Family>> {
    return api<Family>({ id: '', name: '', createdAt: '', members: [] }, async () => {
      ...
      return toFamily({ ...family, member_id: '', role: '' }, members.map(toMember))
    })
  },
  ...
  async getRecords(catId?: string): Promise<APIResponse<RecordDTO[]>> {
    return api<RecordDTO[]>([], () => recordApi.list(familyId(), catId && catId !== 'all' ? { cat_id: catId } : {}))
  },

  async parseAI(text: string): Promise<APIResponse<AIParseSession>> {
    return api<AIParseSession>(emptyParseSession(text), async () => toParseSession(await aiApi.parse(familyId(), text)))
  },
